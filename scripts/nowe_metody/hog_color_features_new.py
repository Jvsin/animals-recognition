...
def policz_hog_kolor_dla_obrazka(obraz_rgb,orientations=9,pixels_per_cell=(8, 8),cells_per_block=(2, 2),bins_hist=16):

    #upewniam się, że mamy floaty w zakresie [0, 1]
...
def policz_hog_kolor_batch(lista_obrazow,orientations=9,pixels_per_cell=(8, 8),cells_per_block=(2, 2),bins_hist=16):
    wszystkie_cechy = []

    for obraz in lista_obrazow:
        cechy = policz_hog_kolor_dla_obrazka(obraz,orientations=orientations,pixels_per_cell=pixels_per_cell,cells_per_block=cells_per_block,bins_hist=bins_hist)
